...
from ultralytics.models.experimental import attempt_load
from ultralytics.engine.model import Model
from ultralytics.models.yolo_test import Model
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if flage_visual:
                ...
                for num in range(batch_size):
                    image = imgs[num, :3, :, :].cpu().clone()  # clone the tensor，复制RGB通道
                    image = image.squeeze(0)  # remove the fake batch dimension，去掉batch维度
                    image = unloader(image)
                    image.save('example_%s_%s_%s_color.jpg'%(str(epoch), str(i), str(num)))
                    image = imgs[num, 3:, :, :].cpu().clone()  # clone the tensor，取当前样本的前 3 个通道（通常是 RGB），确保数据在 CPU 上并创建副本，避免梯度计算干扰。
                    image = image.squeeze(0)  # remove the fake batch dimension，去掉 batch 维度（如果多余）
                    image = unloader(image) # 转换为 PIL 格式
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default='yolov5l.pt', help='initial weights path')#预训练权重（default置空不采用）
    parser.add_argument('--cfg', type=str, default='./models/transformer/yolov5l_fusion_add_FLIR_aligned.yaml', help='model.yaml path')#配置文件路径
    ...
    parser.add_argument('--rect', action='store_true', help='rectangular training')#是否采用矩阵推理方式（图像resize到640时减少冗余部分）
    ...
    parser.add_argument('--multi-scale', action='store_true', help='vary img-size +/- 50%%')#多尺度训练，随机多个尺寸的分辨率训练，鲁棒性更强
    ...
    parser.add_argument('--quad', action='store_true', help='quad dataloader')#开启后若图像大于640取得很好效果，否则可能取得反向效果
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            ...
            # 自动化超参数优化 的核心，它通过进化算法（类似遗传算法）不断调整超参数，最终找到性能最优的参数配置。
            # Train mutation
