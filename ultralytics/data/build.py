...
from ultralytics.data.dataset import GroundingDataset, YOLODataset, YOLODualDataset, YOLOMultiModalDataset
...
def build_yolo_dataset(
    ...
    view = str(data.get("view", "")).lower()
    label = str(data.get("label", "")).lower()
    label_dir = data.get("label_dir")
    if not label_dir and label in {"obb", "hbb"}:
        label_dir = "obb_labels" if label == "obb" else "labels"
    ...
    if view == "dual":
        ...
        if rgb_key not in data:
            if mode in data:
                data = {**data, rgb_key: data[mode]}
            else:
                raise ValueError(f"dual view requires '{rgb_key}' or '{mode}' in data.yaml")
        if ir_key not in data and mode not in data:
            raise ValueError(f"dual view requires '{ir_key}' or '{mode}' in data.yaml")
        if ir_key not in data:
            if data.get("ir_dir") or data.get("list_dir"):
                data = {**data, ir_key: data[mode]}
            else:
                data = {**data, ir_key: None}
        data = {**data, "channels": 6}
        return YOLODualDataset(
            rgb_path=data[rgb_key],
            ir_path=data[ir_key],
            label_dir=label_dir,
            imgsz=cfg.imgsz,
            batch_size=batch,
