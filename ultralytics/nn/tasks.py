...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        self.dual = bool(self.yaml.get("dual", False))
        self.dual_shared = bool(self.yaml.get("dual_shared", True))
        self.dual_fuse = str(self.yaml.get("dual_fuse", "cat")).lower()
        self.dual_split = int(self.yaml.get("dual_split", 3))
        self.dual_in_channels = int(self.yaml.get("dual_in_channels", 3))
        self.backbone_len = len(self.yaml.get("backbone", []))
        self.dual_fuse_indices = self._get_dual_fuse_indices() if self.dual else set()

        # Define model
        if self.dual:
            ch = self.dual_in_channels
        self.yaml["channels"] = ch  # save channels
        if nc and nc != self.yaml["nc"]:
            LOGGER.info(f"Overriding model.yaml nc={self.yaml['nc']} with nc={nc}")
            self.yaml["nc"] = nc  # override YAML value
        self.model, self.save = parse_model(deepcopy(self.yaml), ch=ch, verbose=verbose)  # model, savelist
        self.layer_channels = getattr(self.model, "ch", [])
        if self.dual and not self.dual_shared:
            self.backbone_ir = nn.Sequential(*deepcopy(self.model[: self.backbone_len]))
        if self.dual and self.dual_fuse in {"cat_conv", "cat_reduce"}:
            self.dual_fuse_layers = nn.ModuleDict()
            for i in self.dual_fuse_indices:
                c = self.layer_channels[i]
                if self.dual_fuse == "cat_reduce":
                    self.dual_fuse_layers[str(i)] = nn.Conv2d(c * 2, c, 1, 1, 0, bias=False)
                else:
                    self.dual_fuse_layers[str(i)] = nn.Conv2d(c, c, 1, 1, 0, bias=False)
            if self.layer_channels:
                c_out = self.layer_channels[self.backbone_len - 1]
                if self.dual_fuse == "cat_reduce":
                    self.dual_fuse_out = nn.Conv2d(c_out * 2, c_out, 1, 1, 0, bias=False)
                else:
                    self.dual_fuse_out = nn.Conv2d(c_out, c_out, 1, 1, 0, bias=False)
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            dummy_ch = self.dual_split * 2 if self.dual else ch
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, dummy_ch, s, s))])  # forward
    ...
    def _get_dual_fuse_indices(self) -> set[int]:
        """Return backbone indices referenced by head layers (used for dual fusion)."""
        fuse = set()
        for f, *_ in self.yaml.get("head", []):
            if isinstance(f, int):
                if 0 <= f < self.backbone_len:
                    fuse.add(f)
            else:
                for fi in f:
                    if isinstance(fi, int) and 0 <= fi < self.backbone_len:
                        fuse.add(fi)
        return fuse

    def _forward_backbone(self, x, layers, profile=False, visualize=False, dt=None):
        """Run only backbone layers and return last output and saved features."""
        y = []
        for m in layers:
            if m.f != -1:
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        """Forward pass with optional dual-branch shared-backbone fusion."""
        if not self.dual:
        ...
        split = self.dual_split
        if x.shape[1] != split * 2:
            raise ValueError(f"dual input needs {split * 2} channels, got {x.shape[1]}")

        rgb = x[:, :split]
        ir = x[:, split : split * 2]

        rgb_layers = self.model[: self.backbone_len]
        ir_layers = rgb_layers if self.dual_shared else self.backbone_ir
        rgb_x, rgb_y = self._forward_backbone(rgb, rgb_layers, profile, visualize, dt)
        ir_x, ir_y = self._forward_backbone(ir, ir_layers, profile, visualize, dt)

        y = [None] * self.backbone_len
        for i in range(self.backbone_len):
            if i in self.save:
                r, t = rgb_y[i], ir_y[i]
                if r is None or t is None:
                    y[i] = None
                elif self.dual_fuse in {"cat", "cat_conv", "cat_reduce"}:
                    y[i] = torch.cat((r, t), dim=1)
                    if self.dual_fuse in {"cat_conv", "cat_reduce"}:
                        y[i] = self.dual_fuse_layers[str(i)](y[i])
                elif self.dual_fuse == "sum":
                    y[i] = r + t
                else:
                    raise ValueError(f"Unsupported dual_fuse '{self.dual_fuse}'")

        if self.dual_fuse in {"cat", "cat_conv", "cat_reduce"}:
            x = torch.cat((rgb_x, ir_x), dim=1)
            if self.dual_fuse in {"cat_conv", "cat_reduce"}:
                x = self.dual_fuse_out(x)
        else:
            x = rgb_x + ir_x
...
def parse_model(d, ch, verbose=True):
    ...
    dual = bool(d.get("dual", False))
    dual_fuse = str(d.get("dual_fuse", "cat")).lower()
    dual_fuse_indices = set()
    if dual and dual_fuse in {"cat", "cat_conv", "cat_reduce"}:
        for f, *_ in d.get("head", []):
            if isinstance(f, int):
                if 0 <= f < backbone_len:
                    dual_fuse_indices.add(f)
            else:
                for fi in f:
                    if isinstance(fi, int) and 0 <= fi < backbone_len:
                        dual_fuse_indices.add(fi)
    if scales:
        if not scale:
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        if dual and dual_fuse in {"cat", "cat_conv"} and i == backbone_len:
            for idx in dual_fuse_indices:
                ch[idx] *= 2
        m = (
