...
class BaseModel(nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        该函数遍历模型的每一层，并依次执行前向传播。
        如果启用了 profile，它会记录每一层的计算时间。
        如果启用了 visualize，它会保存每一层的特征图以供后续分析。
        如果指定了 embed，它会提取指定层的嵌入向量，并在最后将其拼接返回。
        """
        '''
        模型前向传播一次
        y 用于存储每一层的输出。
        dt 用于存储每层的计算时间（如果启用了 profile）。
        embeddings 用于存储提取的嵌入向量。
        '''
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # 创建一个字典 self.names，存储每个类别的名称（默认为数字形式，例如：0: "0", 1: "1"）
        self.names = {i: f"{i}" for i in range(self.yaml["nc"])}  # default names dict
        self.inplace = self.yaml.get("inplace", True)
        self.end2end = getattr(self.model[-1], "end2end", False)

        # Build strides, 处理 Detect 模块的步长计算
        '''
        获取模型的最后一层 Detect()（通常是目标检测层）。
        如果该层是 Detect 类的实例（包括其子类，如 Segment，Pose，OBB 等），则：
        设置步长（stride）为 256。
        定义 _forward 函数进行一次前向传播，计算步长。
        使用 _forward 计算步长（s / x.shape[-2]）并存储到 m.stride。
        调用 bias_init() 初始化偏置。
        如果不是 Detect 层，则将步长设置为 32。
        '''
        m = self.model[-1]  # Detect()
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            s = 256  # 2x min stride
        ...
        # self.stride = torch.Tensor([32, 64, 128])  # 假设网络有三个尺度的特征图，步长分别是32、64和128
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m is Conv and len(args) == 3 and args[0] == 64 and i == 5:
                c1, c2 = 3, args[0]
            else:
                # 取出输入通道数 c1 和 目标输出通道数 c2
                c1, c2 = ch[f], args[0]
            # 如果 c2 不是类别数（即不是最终分类层）
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                # 在神经网络（如 MobileNet、EfficientNet）中，通常希望通道数为 8、16、32 等倍数，以提高计算效率和 SIMD 计算的适配性。
                # make_divisible 确保通道数是某个 divisor（通常是 8 或 16）的倍数。
                c2 = make_divisible(min(c2, max_channels) * width, 8) # 进行通道调整
            if m is C2fAttn:
            ...
            if m is C3k2:  # for M/L/X sizes
                legacy = False
                if scale in "mlx": # M/L/X 版本中，既然不缺算力，所有的 C3k2 结构都被改为 C3k，更强的特征提取能力，提升检测效果，尤其是小目标
                    args[3] = True
        elif m is AIFI:
            args = [ch[f], *args]
