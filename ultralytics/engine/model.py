...
class Model(nn.Module):
    ...
    def _new(self, cfg: str, task=None, model=None, verbose=False) -> None:
        ...
        # 如果 model 不是 None，直接使用 model,否则调用 self._smart_load("model") 来加载模型
        # 确定 model 之后，实例化模型对象，cfg_dict是从 yaml 解析得到的模型配置字典，包含网络架构、超参数等信息。RANK=-1用于分布式训练，确保只在 RANK=-1 时打印日志。
